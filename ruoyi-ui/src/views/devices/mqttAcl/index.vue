  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="访问类型： 1->订阅；2->发布；3->订阅与发布" prop="access">
        <el-input
          v-model="queryParams.access"
          placeholder="请输入访问类型： 1->订阅；2->发布；3->订阅与发布"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="是否允许访问：0-deny，1-allow" prop="allow">
        <el-input
          ...
          placeholder="请输入是否允许访问：0-deny，1-allow"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['devices:mqttAcl:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['devices:mqttAcl:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['devices:mqttAcl:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['devices:mqttAcl:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listMqttAcl, getMqttAcl, delMqttAcl, addMqttAcl, updateMqttAcl } from "@/api/devices/mqttAcl";
...
export default {
  ...
  data() {
    return {
      ...
      // 总条数
      total: 0,
      // mqtt客户的acl规则，符合该规则的发布/订阅才可行表格数据
      mqttAclList: [],
      // 弹出层标题
      title: "",
      // 是否显示弹出层
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listMqttAcl(this.queryParams).then(response => {
        this.mqttAclList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加mqtt客户的acl规则，符合该规则的发布/订阅才可行";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.loading = true;
      ...
      getMqttAcl(id).then(response => {
        this.loading = false;
        this.form = response.data;
        this.open = true;
        this.title = "修改mqtt客户的acl规则，符合该规则的发布/订阅才可行";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateMqttAcl(this.form).then(response => {
            ...
            addMqttAcl(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除mqtt客户的acl规则，符合该规则的发布/订阅才可行编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
        return delMqttAcl(ids);
      }).then(() => {
        this.loading = false;
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('devices/mqttAcl/export', {
        ...this.queryParams
